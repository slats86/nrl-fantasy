...
test('generated official snapshot is structurally complete and source-attributed',()=>{
  const data=JSON.parse(fs.readFileSync(require.resolve('../public/team-news.json'),'utf8')),rounds=new Set(data.teamLists.map(x=>x.round));assert.ok(data.availability.length>50);assert.ok(data.teamLists.length>=7);assert.equal(data.receivedClubCount,data.expectedClubCount);assert.ok(data.sourceHash);assert.ok(data.availability.every(x=>x.publisher&&x.sourceUrl&&x.checkedAt&&x.accuracy));assert.ok(data.availability.some(x=>x.type==='rest'));assert.ok(data.suspensions.every(x=>x.type==='suspension'));assert.ok(data.lateMail.length>0);assert.ok(rounds.has(data.latestRound));assert.ok([...rounds].every(round=>round<=data.latestRound&&round>=data.latestRound-1));assert.ok(data.changes.every(x=>!x.summary.match(/^(.+) out — \1 added/)));
});
