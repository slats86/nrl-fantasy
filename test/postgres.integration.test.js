...
test('PostgreSQL migrates JSON and persists all entities transactionally', {skip: !process.env.TEST_DATABASE_URL}, async () => {
  ...
  try { await db.migrate(); const state=await db.load(); assert.equal(state.users['owner@example.com'].teamId,'TEAM1'); assert.equal(state.leagues.ABC123.teams[0].picks[1].FB,123); assert.equal(state.scores['1:123'],42); }
